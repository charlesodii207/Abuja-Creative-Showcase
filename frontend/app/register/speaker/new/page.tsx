...
export default function SpeakerRegistrationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/register/speaker`, {
        method: "POST",
  ...
  return (
    <main className="mx-auto max-w-xl px-6 py-20 md:py-28">
      ...
      <p className="mt-3 text-muted">
        Share your expertise at ACS. Applying does not guarantee a spot.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-10 rounded-2xl border border-gold/20 bg-ink-raised px-6 py-8 sm:px-8 sm:py-10">
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm text-muted">Topic</label>
            <input
              required
              type="text"
              placeholder="e.g. Building a Sustainable Creative Business"
              value={form.topic}
          ...
          <div>
            <label className="block text-sm text-muted">Headshot URL</label>
            <input
              type="url"
              placeholder="https://link-to-your-photo.com"
